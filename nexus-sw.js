...
// v16.6.0 (2026-08-17) — 홈 자산 그래프를 **v16.0 방식으로 복원**.
//   면 채움 라인 + 현재값 점, 기간 1주/1달/전체. 4탭·$눈금·십자선·매매 마커·
//   손익 스트립 전부 제거. 데이터 수집 백엔드는 무접촉(화면만 복원).
//   PC 원본과 동시 diff 이식.
// v16.6.1 (2026-08-17) — v16.6 이식의 부수 피해 복구.
//   그래프 CSS 아래 붙어 있던 '보유 리스트'·'달력' 규칙 90줄이 함께 지워져
...
// v16.7.1 (2026-08-18) — 기간 탭 의미 정정. 세 탭 모두 '기간 창 + 일 단위 점'
//   이고 집계는 없다: 주간=5일 전~오늘 · 월=이번 달 1일~오늘 · 전체=기록 시작~오늘.
//   (v16.7 의 '월별 = 각 달 마지막 기록값' 집계는 의도 오해였다.)
const CACHE_VERSION = 'v16.7.1';
...
function isApiRequest(url) {
  ...
  // v16 — 일지 인덱스는 매일 갱신되는 데이터. 정적 캐시하면 묵는다.
  if (p.indexOf('/dashboard/nexus-journal.json') === 0) return true;
